...
function setupDevServer(config) {
  config.setupMiddlewares = (middlewares, devServer) => {
    ...
    const isAllowedOrigin = (origin) => {
      ...
      // (Removed emergent.sh / emergentagent.com allowances to clear branding)
      ...
      try {
        ...
        Object.entries(changesByFile).forEach(([fileName, fileChanges]) => {
          ...
          try {
            ...
            execSync(`git -c user.name="visual-edit" -c user.email="support@peeraxis.local" add "${targetFile}"`);
            execSync(`git -c user.name="visual-edit" -c user.email="support@peeraxis.local" commit -m "visual_edit_${timestamp}"`);
          } catch (gitError) {
